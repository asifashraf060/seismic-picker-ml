...
def train_model(model, train_loader, val_loader, num_epochs=50, learning_rate=0.001):
    """Train the adaptive model with transformer attention"""
...
def plot_model_predictions(model, dataset, num_examples=1, save_path='predictions_transformer', 
    ...
    with torch.no_grad():
        for i in range(num_examples):
            ...
            plt.title(f'Model Predictions {i} - {feature_mode} Mode (Transformer)', fontsize=14)
            plt.tight_layout()
...
def main(use_physics_features=True, use_max_amplitude=True, window_length=None,
    ...
    """
    Main execution function for ML pipeline with Transformer Self-Attention
    ...
    print("="*80)
    print("TRANSFORMER SELF-ATTENTION SEISMIC PHASE PICKER - ML PIPELINE")
    print("="*80)
    ...
    # Visualize features
    print(f"\n📊 VISUALIZING {feature_mode.upper()} FEATURES WITH TRANSFORMER")
    print("-" * 50)
    
    print('Visualizing and saving waveform features...')
    '''
    visualize_features(val_dataset, 
                      save_path='features_transformer',
                      use_physics_features=use_physics_features, 
                      use_max_amplitude=use_max_amplitude, 
                      window_length=window_length)
    '''
    ...
    print('\n')
    print("-" * 50)
    print(f"INITIALIZING TRANSFORMER {feature_mode.upper()} MODEL")
    print("-" * 50)
    ...
    print('\n')
    print("-" * 50)
    print("🚀 STARTING TRAINING WITH TRANSFORMER SELF-ATTENTION")
    print("-" * 50)
    ...
    try:
        ...
        print("✅ Loaded best transformer model from training")
    except:
    ...
    print(f"\n🎯 PICK ACCURACY RESULTS - TRANSFORMER {feature_mode.upper()} MODE")
    print("=" * 60)
    ...
    print("\n📊 CREATING TRANSFORMER VISUALIZATIONS")
    print("-" * 50)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Training and Validation Loss - Transformer {feature_mode}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title(f'Distribution of Pick Time Errors - Transformer {feature_mode}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    save_name = f'transformer_{"physics_informed" if use_physics_features else "raw_waveform"}_training_results.png'
    plt.savefig(save_name, dpi=150, bbox_inches='tight')
    plt.show()
    
    # Plot model predictions
    plot_model_predictions(model, val_dataset, num_examples=len(val_dataset), 
                          save_path='predictions_transformer', use_physics_features=use_physics_features)
    ...
    if use_physics_features and hasattr(model, 'feature_weights') and model.feature_weights is not None:
        print(f"\n🎛️ Final learned feature weights (Transformer):")
    ...
    # Save model
    model_name = f'transformer_{"physics_informed" if use_physics_features else "raw_waveform"}_phase_picker.pth'
    torch.save(model.state_dict(), model_name)
    print(f"\n💾 Model saved as '{model_name}'")
    
    print(f"\n🎉 TRAINING COMPLETE - TRANSFORMER {feature_mode.upper()} MODE!")
    print("=" * 80)
